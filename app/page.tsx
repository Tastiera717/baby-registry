...
import { Trash2 } from "lucide-react"; // Assicurati di avere lucide-react installato o usa un emoji
...
export default function BabyRegistry() { 
  ...
  return ( 
    <div className="min-h-screen w-full flex flex-col items-center p-4 relative font-dreaming text-blue-800 overflow-x-hidden"> 
            ...
            {photos.map((p) => ( 
              <div key={p.id} className="relative group">
                ...
                {myPhotoIds.includes(p.id) && (
                    <button onClick={(e) => deletePhoto(p.id, e)} className="absolute top-1 left-1 bg-red-500/80 text-white rounded-full p-1 shadow-md">
                        <Trash2 size={12} />
                    </button>
                )}
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto pr-1"> 
            {messages.map((m) => (  
              <div key={m.id} className="bg-white border border-blue-50 rounded-xl p-3 text-sm flex justify-between items-start gap-2">
                  <span>{m.text}</span>
                  {/* Tasto Cancella Messaggio */}
                  {myMessageIds.includes(m.id) && (
                      <button onClick={() => deleteMessage(m.id)} className="text-red-300 hover:text-red-500 transition-colors">
                          <Trash2 size={14} />
                      </button>
                  )}
              </div> 
      ...
      {/* Popups e Modali rimangono invariati */}
      {showThanks && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center px-6 pointer-events-none">
          <div className="bg-white/95 border-2 border-blue-100 rounded-2xl p-4 shadow-xl flex items-center gap-2 animate-center-pop-mobile">
            ...
            <div className="text-blue-800 font-bold text-sm whitespace-nowrap flex items-center gap-1">Grazie mille da Michi! 💙</div>
          </div>
        </div>
      )}
